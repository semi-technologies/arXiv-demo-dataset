...
def get_client():
    client = weaviate.Client("http://localhost:8080", client_config=config)
...
def add_and_return_authors(data, batch_size=512, max_papers=float('inf'), max_import_items=float('inf'), authors_dict={}, start=0):
...
def add_and_return_journals(data, batch_size=512, max_papers=float('inf'), max_import_items=float('inf'), journals_dict={}, start=0):
    batch = weaviate.ThingsBatchRequest()
...
def add_and_return_papers(data, categories_dict, journals_dict, authors_dict, max_papers=1000, batch_size=512, max_import_items=float('inf'), start=0):
    batch = weaviate.ThingsBatchRequest()
...
if __name__ == "__main__":
    ...
    print('Completed loading ArXiv dataset! ...')
    categories_dict = get_ids_of_categories()
    journals_dict = add_and_return_journals(data, max_papers=max_papers, start=start)
    time.sleep(2)
    authors_dict = add_and_return_authors(data, max_papers=max_papers, start=start)
    time.sleep(2)
    paper_authors_uuids_dict = add_and_return_papers(data, categories_dict, journals_dict, authors_dict, max_import_items=max_papers, start=start)
    time.sleep(2)
    add_wrotepapers_cref(paper_authors_uuids_dict)
